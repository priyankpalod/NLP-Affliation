...
files_list = ["acl1.xml"]
root_folder = '/home/tulasi-maggu/Desktop/NLP/test_pdfs/'
...
def isAffiliation(y,fs):
    ...
    if x.find("Universit")!=-1 or x.find(" Labs")!=-1 or x.find("Laboratories")!=-1 or x.find("Institut")!=-1 or x.find("Research")!=-1 or x.find("College")!=-1 or x.find("Corporat")!=-1 or x.find("Academy")!=-1 or x.find("Normale")!=-1 or x.find("Polytechnique")!=-1 or x.find("Politecnic")!=-1 or x.find("Universidad")!=-1:
        t = x.split(' ')
        countsmall = 0
        countall = 0
        for word in t:
    ...
    for i  in range(len(texts)):
        ...
        for j in range(len(tokens)):            
            if type(tokens[j].text) is unicode:
                ...
                if isinstance(word, types.NoneType) and flag==True:
                    continue
